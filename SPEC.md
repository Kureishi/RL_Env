...
| M46 | v0.43   | 57     | A47 | tests/test_research_v043.py |
| M47 | v0.44   | 58     | A48 | tests/test_surfaces_v044.py |
...
## 57. Research decision surfaces (v0.43)

A finished run already logs everything the improver saw — every candidate's spec, score, verdict, the champion chain, the bandit's field stats — but the views so far have been *linear* (curves, strips, timelines). A researcher on a niche, specialized task needs the search as a *structure*: where was it explored, which knobs matter for this data, where is the acceptance boundary, and what does the algorithm currently believe. v0.43 turns the history already logged into four decision surfaces, in a new leaf core module `research.py` (stdlib only — streamlit-free, 3 / 23.1) plus four SVG renderers in `plotting.py`. All four are pure derivations (G2): no new logged field, no loop change; the app renders the block in the Results view (57.5); the report (22) and the share bundle (47.4) are untouched, so every byte-identity pin holds.

### 57.1 The spec-lineage graph

`research.spec_lineage(entries)` reconstructs the search as a graph (a DAG, in practice a tree — every candidate mutates the champion in force, so each node has exactly one parent). The parent link is reconstructed from log order: the reset baseline seeds the champion; every accepted candidate becomes it; a curriculum step-up (20.1) re-pins the *score* (`new_baseline_score`) but carries the same spec over, so the champion — and the graph — is untouched; `invalid_spec` rows (25.4) carry no `spec_hash` / score and are excluded (they never entered the search space). Each node is `{hash, score, accepted, label}` (the label is `family <hash6>`, degrading to `spec <hash6>` when the spec is not a dict carrying a `model_family`); each edge is `{src, dst, accepted, delta, fields}` with `delta` = the candidate's score minus the champion score in force (None when either is missing) and `fields` the mutated fields.

`plotting.svg_spec_lineage(lineage, width=760, row_h=64, ...)` lays it out as a tidy tree (DFS leaf order on x, internal nodes centered over their children, depth on y): cubic-bézier edges — green (`_ACCEPTED`) for accepted mutations, grey (`_REJECTED`) for rejected — with stroke width `1.5 + 3.5·min(1, |Δ|/10)` (edge weight = score delta); node boxes carry the label + the candidate's score, filled by verdict; a legend and an `n specs / n mutations` summary; a `<title>` on every edge (mutated fields, verdict, Δscore) and every node (label, score, verdict, mutated fields). palette (51.4) / dark (51.4.2) / ARIA (51.4.3) apply; the default call is byte-identical (G2); the empty graph renders the header + `no logged specs (empty run)` (21.2).

### 57.2 Per-field response surfaces

`research.field_response_stats(entries)` is the partial-dependence view over the logged candidates: for every *scored* entry (kind baseline / experiment with a finite `holdout_score` and a dict spec), each `(field, value)` pair of that candidate's **full** spec is credited with the entry's holdout score — the baseline row contributes too (its spec is a point in the same space). Screen (35.1) / curriculum / invalid rows are not scored candidates and are excluded. Values are keyed with the V2 convention (30.2: `None` → `"-"`, lists joined with `,`), and the result `{field: {value: {n, mean, best}}}` is sorted (fields alphabetical, values numeric-first, the 30.2 order).

`plotting.svg_field_response(stats, width=640, row_h=64, ...)` renders one row per field (sorted) and one cell per value (numeric-first, the 30.2 `_fv_sort_key` order): a lane-background cell with a baseline, a vertical bar whose height is the mean (clamped to the 0-100 scale), the mean value above the bar, `value · n` below the baseline, and a `<title>` per cell (`field value: mean (n, best)`). The core "which knobs matter for my data" view. palette / dark / ARIA apply; empty stats render the header + `no scored specs to aggregate` (21.2).

### 57.3 The gate-decision region

`research.gate_region_candidates(entries)` returns every scored candidate as a `(holdout score, gen_gap)` point with its verdict and its running `best_before`, plus the run's `baseline_score` and the §18.5 tolerance: `{baseline_score, tol, candidates: [{score, gen_gap, accepted, reason, best_before}]}`. The running best is reconstructed exactly like `accounting._rejections` (39.2.2) — the baseline seeds it, a curriculum step-up re-pins it, an accepted candidate raises it — and each point's `reason` is `accounting.candidate_reason` (the score → overfit → ci priority), so the region plot and the report's decision accounting cannot drift.

`plotting.svg_gate_region(data, width=640, height=420, ...)` draws the holdout-score × gen-gap plane with the acceptance rule as a *region* instead of an opaque "rejected": a shaded polygon for `{score ≥ baseline} ∩ {gen_gap ≤ (1+tol)·score − baseline}` — the 18.5 overfit penalty drawn with z = 0 — a dashed line for the penalty boundary `gen_gap = tol·score`, a dashed vertical line at the run baseline, and every candidate as a dot — accepted (`_ACCEPTED`), overfit (`_SCORED_REJ`), score (`_BASELINE`), ci (`_LINE`), any other reason (`_REJECTED`) — each with a `<title>` (score, gap, reason, best_before). A legend plus a caption noting that the per-step CI margin (18.6) is a state-dependent shift (z·SE) of this region. palette / dark / ARIA apply; no scored candidates render the header + `no scored candidates yet` (21.2).

### 57.4 The bandit belief bars

`research.bandit_beliefs(field_stats, ucb)` turns the update stream's `field_stats` (26.1) into the per-field learned preference with its uncertainty: for each field with `trials > 0`, the 95% Wilson interval of `p = wins/trials` — `center = (p + z²/2n)/(1 + z²/n)`, `half = z·sqrt(p(1−p)/n + z²/4n²)/(1 + z²/n)`, clamped to [0, 1], with the fixed quantile `z = 1.959964` (a constant, not a knob, 33.2) — plus the final UCB value from `ucb` (26.4; a non-finite / missing reading is None, as for the search policy). Fields with `trials ≤ 0` are skipped; the result `{field: {trials, wins, win_rate, lo, hi, ucb}}` is sorted by field.

`plotting.svg_bandit_beliefs(data, width=640, row_h=44, ...)` renders one row per field: a light band (`_BAND`) spanning `[lo, hi]`, a tick at the win rate (`_LINE`), and a diamond at the UCB (`_BASELINE`) when present; the highest-UCB field is flagged "best-believed" (ties: lexicographically smallest, G2). A `<title>` per row carries the full numbers (`win w/n (p) · 95% CI [lo, hi] · UCB u · best-believed`); the x axis runs 0..`max(1, 1.15·max UCB)`. "What the algorithm currently believes is best, and how sure." palette / dark / ARIA apply; empty data render the header + `no field stats — needs a bandit-policy run (SPEC.md 57.4)` (21.2).

### 57.5 The app surface + exports

The app (a thin renderer, 23.1) renders the block in `_render_result` — shared by the live and restored views (23.2) — under a **Research views** subheader, right after the acceptance-gate views and before the Learning views: the three entry-derived surfaces (57.1-57.3) are computed from the run's `experiments.jsonl` at render time (a missing file is a friendly error, not a crash — 49.4.3 — and the renderers then draw their empty states), and the belief bars (57.4) are computed from the stored `field_stats` + the final `ucb` of the stored update stream — with a caption fallback ("bandit belief bars need a bandit-policy run (field_stats) — SPEC.md 57.4") when the run has no field stats. The Setup tab's palette / dark passthrough (`_palette_kwargs()`, 51.4.4) feeds all four renders; no new Streamlit widget is added (AppTest-safe). The four research functions and the four renderers are exported from the package top level and `__all__` (33.1). The report (22) and the share bundle (47.4) are unchanged — the surfaces are app + core only, so the byte-identity pins (56.6) hold.

### 57.6 Acceptance (A47)

- **research core (57.1-57.4)** — `spec_lineage` is hand-computed (the champion chain: the baseline seeds, an accepted candidate re-champions, a rejected one does not; a curriculum step-up re-pins the score without re-parenting; `invalid_spec` rows are excluded; `delta` is None when a score is missing; empty entries → `{"nodes": {}, "edges": []}`); `field_response_stats` is hand-computed (the per-value n / mean / best over the full spec, the screen / curriculum / invalid exclusion, the `"-"` / `","` value keys, the sort order); `gate_region_candidates` is hand-computed (the `best_before` chain incl. the curriculum re-pin, the reason chain accepted → score → overfit, `baseline_score` + `tol = 0.05`); `bandit_beliefs` is hand-computed (the Wilson 95% interval for 5/10 pinned to 1e-12, the `trials ≤ 0` exclusion, a non-finite / missing `ucb` → None, the field sort).
- **renderers (57.1-57.4)** — each output is valid XML (`xml.dom.minidom` parse): the lineage (node labels + scores, the bézier edge paths, the legend + summary, the tooltips); the response surfaces (the per-cell bar + mean + `n` text, the tooltips); the gate region (the acceptance polygon, the penalty line, the baseline line, the verdict-colored dot tooltips, the empty message); the belief bars (the CI band, the win-rate tick, the UCB diamond, the best-believed flag, the empty message); the okabe / dark re-render; the `ValueError` on a bad palette (51.4.1).
- **app (57.5)** — the source wires the **Research views** subheader, the four svg renderers, and the four research functions (with the friendly-error path and the no-field-stats caption); the default path runs end-to-end (a finished run renders the block; no new widget).
- **exports (57.5)** — the four research functions and the four renderers are in `__all__` (33.1).
- **version + regression** — the version steps to `0.43.0` in both sources (33.1); A1–A46 stay green (no default-path behavior change; the report / share-bundle byte-identity pins untouched); the A25 index advances (43 acceptance rows; `defined == set(range(1, 48))`).

### 57.7 Milestone (M46)

**M46** — v0.43 "B. Visually useful for researchers / niche specialized tasks": the spec-lineage graph (57.1), the per-field response surfaces (57.2), the gate-decision region (57.3), and the bandit belief bars (57.4) — all pure derivations from the history already logged (57.5), byte-identical under defaults (G2) (A47).

---

## 58. The researcher round-2 surfaces (v0.44)

Round 1 (§57) turned the logged history into decision surfaces. Round 2 answers the researcher's next questions — *what is this model bad on, and how big is the model I would ship?* — and assembles the whole story of a run into one exportable artifact. Three items: the per-input difficulty ranking (the last piece of the error-analysis pack, 58.1), the 3-objective frontier score × train-time × model-size (58.2), and the run dossier — one self-contained HTML file combining everything (58.3). All three are pure derivations over artifacts already on disk (G2): no new logged field, no loop change; the report (22) and the share bundle (47.4) stay byte-identical.

### 58.1 The per-input difficulty ranking (the error-analysis pack)

`diagnostics.holdout_difficulty(task, model, n=200)` completes the error-analysis pack (28.2's per-class + confusion exist, 28.3's media gallery exists) — the holdout items ordered easiest → hardest, so a niche task's "what is it hard on, and where" is one ordered list. One forward pass over `task.holdout_rows(n, model)` (the 28.2 protocol — slightly broader than `holdout_diagnostics`, which is softmax-only; the mse-head case is served too):

* **softmax head**: `difficulty` is the SIGNED margin `logit[y] − max_{j≠y} logit[j]` — positive and large = an easy, confident correct call; negative = misclassified (the most negative = the hardest). `correct = argmax == y`.
* **mse head**: `difficulty = max_j |pred_j − y_j|` (small = easy, large = hard); `correct = difficulty ≤ 0.05·max(1, max_j |y_j|)` (a 5% relative tolerance — informational; the difficulty value is the ranking signal).

Returns `{"n", "head", "items": [{index, true, predicted, correct, difficulty}]}` with `items` sorted easiest → hardest (softmax: margin DESCENDING; mse: |pred − y| ASCENDING; ties by index). `None` when the task exposes no `holdout_rows` (episode tasks: cartpole/parity/gridnav/sine — the same applicability rule as 28.2) or the holdout split is empty. Deterministic (one forward pass, no RNG, 28.5); `index` is the position in the holdout split, so the ranking composes with the 28.3 error gallery.

`plotting.svg_difficulty_ranking(data, width=640, row_h=26, ...)` renders one row per item: the `#index true → predicted` label on the left, a lollipop bar from the zero axis to the difficulty value, colored by the row's verdict (`_ACCEPTED` correct / `_REJECTED` misclassified), a `<title>` per row carrying the full numbers. The axis is signed when any difficulty is negative (the softmax margin mode) and non-negative otherwise (the mse `|pred − y|` mode). At most 30 rows render (the head + tail — the informative ends — with a `… n omitted …` marker) so a 200-item holdout stays legible. palette (51.4) / dark (51.4.2) / ARIA (51.4.3) apply; the default call is byte-identical (G2); no items render the header + `no holdout items (non-classification task?)` (21.2).

### 58.2 The 3-objective frontier (score × train-time × model-size)

`config.spec_n_params(spec, state_dim, n_out, grid=None)` counts one spec's parameters — the size axis. `state_dim` / `n_out` come from the task (the spec JSON does not carry them — the same contract as `svg_architecture`); `grid` is the task's `feature_grid` `(C, H, W)`. It counts weights + biases exactly (the npz value count, the `gate.model_size` semantic, 25.5):

* **mlp**: `sizes = [state_dim, *architecture, n_out]`; each layer contributes `fan_in·fan_out + fan_out`.
* **convnet**: mirrors `models.convnet.ConvNet.__init__` exactly — `conv1: c1·C·9 + c1`, `conv2: c2·c1·9 + c2`, `fc: flat·32 + 32` (the same pool arithmetic, `flat = c2·h2p·w2p`), `head: 32·n_out + n_out`.
* **tree / boost / knn**: `None` — their "size" is data-dependent (bagged tree shapes, k at inference), not a parameter count; the 58.2 size axis is incomparable for those candidates (documented).

`None` (not an error) when the inputs are missing, non-finite, or the grid is too small for a valid convnet pipeline (the `ConvNet` would raise `SpecError` — the count degrades instead). Leaf: the `FC_HIDDEN` constant is lazy-imported inside the function so `config` never imports `models` at top level (3's leaf rule — models import config).

`research.frontier3(entries, state_dim=None, n_out=None, grid=None)` attaches the size axis to the 57.2 scored candidates (kind baseline/experiment, a finite `holdout_score`, a dict `spec`) that also carry a finite `train_seconds` (the pair 21.2's `svg_pareto` needs): points `{score, time, size, kind, hash, label, frontier}`. 3D non-dominance: A dominates B iff `A.score ≥ B.score` and `A.time ≤ B.time` and — only when BOTH carry a size — `A.size ≤ B.size`, with at least one strict inequality on a *comparable* axis (score and time are always comparable; a `None` size neither violates nor establishes dominance). The frontier is the set of non-dominated points. Returns `{"points", "frontier" (indices), "unsized"}`. Pure and deterministic (G2); no scored candidates → empty points.

`plotting.svg_frontier3(data, width=980, height=380, ...)` renders the two-panel tradeoff: panel 1 (left) score × train-seconds — every point (the 2-objective view); panel 2 (right) score × parameter count — the sized points only (the unsized ones appear in panel 1 and are counted in the caption). Frontier points are the strong color (`_ACCEPTED`), dominated ones grey (`_REJECTED`); every dot carries a `<title>` (label, score, seconds, size or n/a, verdict) plus a legend and the non-dominance rule caption. palette / dark / ARIA apply; no points render the header + `no scored candidates with train time`; no sized points render panel 2's caption (21.2).

### 58.3 The run dossier

`dossier.build_dossier(run_dir, state_dim=None, n_out=None, grid=None, difficulty=None, gallery=None, per_class_svg=None, confusion_svg=None)` is the "here is the whole story of this run" object — one scrollable, self-contained HTML document (inline `<style>`, no external assets, 22.2; no timestamps — a re-render is byte-identical, G2) combining:

1. **Summary** — the run's headline numbers (the report's table, 22.1).
2. **Recipe** — the canonical `run_config.json` (37.1) as the copy-pasteable `format_recipe` line (47.1) plus the full JSON (fallback: the summary's `run_config` mirror; a caption when the run predates v0.23).
3. **Provenance** — the 56.1 certificate (env facts + the run's identity + the run-config hash) as the SVG card.
4. **Frontier** — the 2-objective `svg_pareto` (21.2) + the 3-objective `svg_frontier3` (58.2), whose size axis comes from `spec_n_params` with the task's `state_dim` / `n_out` / `feature_grid` (None args degrade every size to the data-dependent case).
5. **Spec lineage** — the 57.1 mutation DAG.
6. **Sensitivity** — the 57.2 per-field response surfaces.
7. **Error analysis** — the per-class bars + confusion matrix (28.2, pre-rendered by the caller — the dossier never trains), the 58.1 difficulty ranking, and the 28.3 media error gallery (data-URI thumbnails / waveform SVGs). The section degrades to a caption when the run carries no classification data.

`run_dir` is a finished run dir (`summary.json` + `experiments.jsonl`); a missing summary is a `ValueError` (the `_run_task_and_model` contract, 42). All dynamic text is HTML-escaped; the pre-rendered SVGs are inlined as-is (they are valid XML by their own contract). Pure over the run dir (G2).

### 58.4 The surfaces (app + CLI) + exports

The app (a thin renderer, 23.1) renders the difficulty ranking in the Learning views — shared by the live and restored views (23.2) — right after the confusion matrix, when the task exposes holdout rows. The `finish()` result gains the additive keys `difficulty` / `difficulty_svg` plus the task geometry the dossier needs (`state_dim` / `n_out` / `grid`), and the Artifacts row gains a fifth download button (**dossier.html**): the build runs in a try/except (the 49.4.3 pattern — a friendly caption, never a page crash) and the button renders only on success. No new Streamlit widget beyond the one download button (AppTest-safe).

The CLI gains a subcommand (the 47.1/47.2 conventions: `--run` + `--out`, `--config`, the `_EP_DOSSIER` epilog, `RawDescriptionHelpFormatter`): `autorefine dossier --run DIR [--out FILE]` — it reconstructs task + model via `_run_task_and_model` (42), pre-renders the 28.2 diagnostics SVGs, pulls the 28.3 gallery from `task.holdout_errors` (media tasks only), and writes `<run_dir>/dossier.html` by default, printing `dossier : <path>`. rc 0 on success; rc 1 when the run dir is missing or broken (the `_cmd_share` contract, 47.4.4).

Exports: `spec_n_params` (config), `holdout_difficulty` (diagnostics), `frontier3` (research), `svg_difficulty_ranking` / `svg_frontier3` (plotting), `build_dossier` (dossier) — all at the package top level and in `__all__` (33.1). The report (22) and the share bundle (47.4) are untouched — the surfaces are app + CLI + core only, so the byte-identity pins (56.6) hold.

### 58.5 Acceptance (A48)

- **58.1 core** — `holdout_difficulty` is hand-computed: a softmax task with known logits gives the exact signed margins (positive = easy correct, negative = misclassified, the correct/incorrect flags), the mse-head case (`|pred − y|` ascending, the 5% tolerance rule), the ordering (margin DESC / |err| ASC, ties by index), and `None` for a task without `holdout_rows` or with an empty split.
- **58.2 core** — `spec_n_params` is hand-computed: mlp arch (8,16) with state_dim 4, n_out 2 → 218; convnet arch (4,8) on grid (1,8,8), n_out 2 → 690; grid (1,4,4) → None (the pipeline invalid); tree/knn → None; missing state_dim / n_out → None. `frontier3` is hand-computed: the scored-candidate rule, the 3D non-dominance incl. a `None`-size point (a bigger, slower, sized point is dominated by a smaller, faster unsized one; the two best stand), screen-kind rows excluded.
- **renderers (58.1-58.2)** — `svg_difficulty_ranking` + `svg_frontier3` are valid XML (`xml.dom.minidom` parse): the lollipop rows (labels, verdict colors, the tooltips), the head+tail cap + the omitted marker, the signed / non-signed axis; the two panels (sized / unsized points, the frontier vs dominated colors, the tooltips, the legend, the caption), the empty states; the okabe / dark re-render; the `ValueError` on a bad palette (51.4.1).
- **dossier (58.3)** — `build_dossier` over a finished run dir: two renders byte-identical (G2), every section present (Summary / Recipe / Provenance / Frontier / Spec lineage / Sensitivity / Error analysis), the error-analysis pack degrading to the caption when no classification data; `ValueError` on a broken run dir.
- **surfaces (58.4)** — the app's source wires the difficulty surface + the fifth artifacts button (with the friendly-error path); the CLI's source wires the `dossier` subcommand (the `_cmd_dossier` / `_EP_DOSSIER` / `--run` / `--out` tokens) + the `build_dossier` import; one light AppTest renders the new surface end-to-end.
- **exports (58.4)** — the six new names are in `__all__` (33.1).
- **version + regression** — the version steps to `0.44.0` in both sources (33.1); A1–A47 stay green (no default-path behavior change; the report / share-bundle byte-identity pins untouched); the A25 index advances (44 acceptance rows; `defined == set(range(1, 49))`).

### 58.6 Milestone (M47)

**M47** — v0.44 "B. Visually useful for researchers / niche specialized tasks (round 2)": the per-input difficulty ranking completing the error-analysis pack (58.1), the 3-objective score × train-time × model-size frontier (58.2), and the run dossier — one self-contained HTML artifact combining the recipe, provenance, frontier, lineage, sensitivity, and error analysis (58.3) — all pure derivations over artifacts already on disk (58.4), byte-identical under defaults (G2) (A48).

---

## 59. Parameters: visually interpreted and modified (v0.45)

The spec space was *read* (the spec chips, the §50 architecture diagram, the §57 per-field response surfaces) but never *steered*. This round splits the parameters into the two real layers they already have — the **model-architecture** layer (the `SPEC_FIELDS` registry, 36.2) and the **loop** layer (the `KNOBS` registry, 33.2) — and gives each a control surface. Three additive items: a read-only **parameter inspector** that makes every knob's current value, space, baseline, best-seen, and measured effect legible (59.1); **steering verbs** — `pin` / `bias` / `constrain` — that turn the autonomous loop into a human-in-the-loop search (59.2); and a **manual/expert mode** where the researcher sets the exact spec and the loop validates + reports instead of discovering (59.3). All opt-in: `AutoRefineEnv(steering=None)` (the default) is the byte-identical pre-v0.45 path (G2), and the policies' `exclude_fields=()` default is the exact pre-v0.45 seeded proposal stream (A1–A4 pins green).

### 59.1 The parameter inspector

`steering.parameter_inspection(entries, best_spec=None, run_config=None, field_stats=None, ucb=None)` is the inspector's data — one row per field over the **two real layers**, in registry order (deterministic, G2): the 15 architecture rows (the `SPEC_FIELDS` registry, 36.2) followed by the 9 loop rows (the `KNOBS` registry, 33.2) — **24 rows total**. Every row carries `name`, `layer` (`"architecture"` / `"loop"`), `spec_ref`, `current`, and `baseline`; the architecture rows additionally carry `kind`, `space` (tuples rendered as lists), `families`, and the measured effect recomposed from the already-computed §57 derivations:

- **best-seen + Δ** from `research.field_response_stats` (57.2): the `(field, value)` pair with the highest mean holdout score is the *best-seen* value, `best_seen_mean` is that mean, `best_seen_n` its sample count, and `delta` is that mean minus the *baseline value's* mean (the value in `DEFAULT_SPEC`) — `None` when the baseline value was never scored;
- **win rate + CI + UCB** from `research.bandit_beliefs` (57.4): `win_rate`, the 95% Wilson interval (`ci_lo` / `ci_hi`), and the final UCB.

The loop rows carry `cli` (the flags, 33.2) and `app_widget` (the app control, 33.2) plus `current` from `run_config`; loop params are not bandit arms, so `win_rate` / `ci_lo` / `ci_hi` / `ucb` / `best_seen` / `delta` are `None`. Pure derivation, no new logged field (G2): `KNOBS` and `research` are imported inside the function (the `advanced.py` local-loader pattern — this module's top level never imports `improver.meta_env`, so there is no import cycle); missing inputs degrade to `None` (an old run without `field_stats` still renders every row).

The app (a thin renderer, 23.1) renders it in `_render_result` — shared by the live and restored views (23.2) — under a **Parameter inspector (SPEC.md 59.1)** subheader, right after the §57 bandit-beliefs block and before the Learning views: two `st.columns` (architecture / loop), each row via `_inspector_field` — display-only (a family/kind/§ref caption, the current·baseline·best-seen·Δ caption, `st.progress(win_rate)` + the CI when present, the space caption for architecture rows; the §ref / current·baseline·cli·widget caption for loop rows). No new Streamlit widget (AppTest-safe); the whole block is wrapped in a try/except (the 49.4.3 pattern — a friendly caption, never a page crash).

### 59.2 The steering verbs (`pin` / `bias` / `constrain`)

`steering.SteeringState` is a frozen, JSON-safe record of the human-in-the-loop rules over the spec space. Three tuple fields, each a `(field, ...)` pair list:

- **`pins`** — `(field, value)` pairs: the field is **force-set** to `value` on *every* candidate **and** on the reset baseline (59.2.2);
- **`biases`** — `(field, value)` pairs: a mutation of `field` is **redirected** to `value` (a candidate already equal — or not mutated — is untouched);
- **`constraints`** — `(field, (value, ...))` pairs: a candidate whose value is **outside** the allowed set is rejected as `invalid_spec` (logged, 25.4 — honest and visible, not a silent rewrite).

Validation is **registry-level** (the C2 knob-registry pattern, 33.2): the field must be in `improver.specspace.SPEC_FIELDS` (G2, 36.2) — `SteeringState.pin/bias/constrain` and `SteeringState.from_dict` reject an unknown field with a `ValueError` naming the registry — and every value must pass the field's own validator (a per-field, loud `ValueError`). The builder methods are pure (they return a **new** state — the immutable RunConfig 37.1.2 pattern) and a second rule for the same field **replaces** the first (last one wins). `SteeringState.active` is `True` when any rule exists (the env's fast path); `to_dict` / `from_dict` are the JSON-safe pair (lists of `[field, value]` for pins/biases and `[field, [value, ...]]` for constraints), re-validating on load (a mutated / unknown dict is rejected).

`steering.apply_steering(spec_dict, state, champion=None)` enforces the rules on one candidate dict (pure — returns a new dict, never mutates the input): **pins** are force-set unconditionally; **biases** redirect only a *mutated* field — one whose value differs from `champion` (the champion in force) — to the biased value; **constraints** produce one violation string per field whose value is outside the allowed set (the caller rejects it as `invalid_spec`, 25.4). `state=None` (or an empty state) returns `dict(spec_dict), []` — the pre-v0.45 path exactly (G2).

**Environment integration** (`improver/meta_env.py`, 59.2): `AutoRefineEnv` accepts `steering: SteeringState | None = None` (a non-`SteeringState` non-`None` is a construction-time `ValueError`, the `stop_check` fail-loud style). Two effects: (1) the **reset baseline** is `DEFAULT_SPEC` with the *pins* force-set (a pin combination that breaks the baseline is a construction-time `ValueError`; no pins → `DEFAULT_SPEC` exactly — the pre-v0.45 path, G2); (2) on every **step**, `apply_steering` is applied to the proposed candidate with the current champion — a pin-rewritten spec that then fails validation is rejected as `invalid_spec` (25.4), a constraint violation rejects it too. `None` steering is the pre-v0.45 step exactly.

**Policy integration** (`improver/policy.py` + `improver/bandit.py`, 59.2): `SearchPolicy` and `BanditPolicy` accept `exclude_fields: tuple = ()` — the steering **pins** drop their field from the mutation-target pool (an all-excluded pool falls back to the full set, so a pin covering every field cannot deadlock the search). The empty default is the exact pre-v0.45 seeded proposal stream (A1–A4 pins green). `AutoRefineGymEnv` / the CLI `_drive` / `DashboardRunner.start` all pass `exclude_fields` from `steering.pins`.

**Surfaces (59.4)**: the CLI's `run` and `fit` gain repeatable `--pin FIELD=VALUE` / `--bias FIELD=VALUE` / `--constrain FIELD=V1,V2` flags (helpers `_steering_value` / `_steering_token` / `_steering_constrain_token` / `_steering_from_args`; all three empty → `steering=None`, the pre-v0.45 path; a bad token is a `ValueError` → rc 1); `runconfig` carries `pins` / `biases` / `constraints` (in `_FIELDS`, `_OPTIONAL_FIELDS` — absent → `[]`, so a pre-v0.45 `run_config.json` still loads — and `_VALIDATORS`), `from_env` reads `env.steering.to_dict()`, and `fit_recipe` / `runconfig_to_flags` round-trip them (`--pin/--bias/--constrain` flags and `field=value` lists respectively); `DashboardRunner` accepts `steering` (a non-`SteeringState` is a `ValueError`), threads it through `start` (the policy `exclude_fields`) and `_sweep_params` (the seed-sweep, 31.2). The app's Setup-tab expander builds the rules via verb / field / value widgets (`stg_verb` / `stg_field` / `stg_val_{field}`) into `st.session_state["steering_rules"]`, the Run button builds `SteeringState.from_dict` (a `ValueError` is a friendly `st.stop()`), and passes `steering` to `_run`.

### 59.3 Manual / expert mode

`steering.manual_spec(values)` builds the exact spec from a `field -> value` override map on top of `DEFAULT_SPEC`: every key is registry-validated (an unknown field is a `ValueError` naming the registry, 59.2.1), every value passes its registry validator, and the *combination* then passes `ModelSpec` validation (an mlp depth-4 architecture or a bad tree depth is a `SpecError` — the same typed error the loop rejects, 25.4). An empty `values` dict is `DEFAULT_SPEC` itself.

`steering.train_manual(task_name, values, seed=7, task_config=None, max_train_seconds=None)` trains + evaluates **one exact spec** — `manual_spec` → `train_from_task` → `evaluate_full`, the exact pattern of `advanced.retrain_spec` (49.2.1). No loop, no budget, **no run dir** (read-only, the 49.2.1 semantics): the researcher knows the architecture and wants the loop to *validate + report*, not discover. `task_config` (e.g. `{"path", "label"}` for the csv task) is the optional data-driven constructor kwargs (22.1); an unknown task name is a `ValueError` (the registry, 15). Returns the JSON-safe dict `{"spec", "score", "gen_score", "gen_gap", "train_seconds", "final_loss", "time_capped"}`.

**Surfaces (59.4)**: the CLI gains a `manual` subcommand (`_cmd_manual`, `_EP_MANUAL` epilog, the 47.1/47.2 conventions — `--task` + repeatable `--set FIELD=VALUE` + `--seed`, `--config`): it builds the override map (last one wins per field), trains once, and prints the result as JSON (rc 0 on success, rc 1 on a `ValueError`). The app's Advanced tab gains a **manual/expert** expander: one `st.selectbox` per registry field (keys `manual_{field}`, defaults = `DEFAULT_SPEC` updated with the run's `best_spec`), a **Train this exact spec** button (`manual_button`) that runs `manual_spec(_mvals)` for early per-field / combination validation (a `ValueError` is a friendly `st.error`) and then `train_manual(task, _mvals, seed=_mseed, task_config=_mtaskcfg)` with the result in an `st.dataframe` (an `OSError` is a friendly error, the 49.4.3 pattern). No new run dir (AppTest-safe).

### 59.4 Surfaces + exports

Exports: `SteeringState`, `apply_steering`, `manual_spec`, `parameter_inspection`, `train_manual` (steering) — all at the package top level and in `__all__` (33.1). The report (22) and the share bundle (47.4) are untouched — the surfaces are app + CLI + core only, so the byte-identity pins (56.6) hold.

### 59.5 Acceptance (A49)

- **59.1 core** — `parameter_inspection` is hand-computed: 24 rows (15 architecture + 9 loop) in registry order; the architecture row for a field carries its `kind` / `space` (tuples as lists) / `families` / `spec_ref`, the `current` value from `best_spec`, the `baseline` from `DEFAULT_SPEC`, and — over a fixture with scored entries — the best-seen value / mean / n and the Δ vs the baseline value's mean (the §57.2 re-derivation), the win rate / Wilson CI / UCB (the §57.4 re-derivation); the loop row carries `cli` / `app_widget` / `current` from `run_config` / `baseline` from `KNOBS`, with `win_rate` / `ci` / `delta` / `best_seen` all `None`; empty `entries` → every row present with the effect fields `None` (the old-run degradation).
- **59.2 core** — `SteeringState` is hand-computed: an unknown field or a bad value on `pin` / `bias` / `constrain` is a `ValueError` naming the registry; an empty `constrain` set is a `ValueError`; a second rule for the same field **replaces** the first (last one wins); `to_dict` / `from_dict` round-trips byte-exact and re-validates (a mutated / unknown dict is a `ValueError`). `apply_steering` is hand-computed: pins force-set unconditionally; a bias redirects only a *mutated* field (a candidate equal to the champion, or already the biased value, is untouched); a constraint violation yields the exact string and an in-set value does not; `None` / an empty state returns `(dict(spec), [])` (the input is not mutated).
- **59.3 core** — `manual_spec` is hand-computed: an empty dict is `DEFAULT_SPEC`; an unknown field is a `ValueError`; a bad combination (e.g. an mlp depth-4 architecture) is a `SpecError`. `train_manual` raises `ValueError` on an unknown task, and one tiny parity train returns the 7-key dict (`spec` / `score` / `gen_score` / `gen_gap` / `train_seconds` / `final_loss` / `time_capped`).
- **integration (59.2)** — the env with a **pin** changes the reset baseline (the pinned field is force-set) and the step force-sets it on candidates; a **constraint** rejects an out-of-set candidate as `invalid_spec` (logged); `None` steering is the pre-v0.45 step exactly (G2). The policies with `exclude_fields` drop the pinned field from the mutation pool while the default stream is unchanged (A1–A4). `runconfig` round-trips: an old dict missing `pins` / `biases` / `constraints` loads with `[]` (back-compat); `from_env` reads `env.steering.to_dict()`; `fit_recipe` / `runconfig_to_flags` emit the `--pin/--bias/--constrain` flags and `field=value` lists.
- **surfaces (59.4)** — the CLI's source wires the `manual` subcommand + the `--pin/--bias/--constrain` flags (the `_cmd_manual` / `_EP_MANUAL` / `_steering_from_args` tokens); the dashboard's source wires the `steering` kwarg + the `exclude_fields` pass-through + `_sweep_params`; the app's source wires the `parameter_inspection` inspector (the `Parameter inspector (SPEC.md 59.1)` subheader + `_inspector_field`), the steering expander (`stg_verb` / `stg_field`), and the manual expander (`manual_button` / `train_manual` / `manual_spec`); one light AppTest renders the inspector with no exception.
- **exports (59.4)** — the five new names are in `__all__` (33.1).
- **version + regression** — the version steps to `0.45.0` in both sources (33.1); A1–A48 stay green (no default-path behavior change; the report / share-bundle byte-identity pins untouched); the A25 index advances (45 acceptance rows; `defined == set(range(1, 50))`).

### 59.6 Milestone (M48)

**M48** — v0.45 "C. Parameters: visually interpreted and modified": the parameter inspector over both the architecture and loop registries (59.1), the human-in-the-loop steering verbs `pin` / `bias` / `constrain` (59.2), and the manual/expert mode — set the exact spec and the loop validates + reports (59.3) — all opt-in (59.4), byte-identical under defaults (G2) (A49).

## 60. Parameters: visually interpreted and modified — what-if & comparison (v0.46)

The spec space was *interpreted* (the §57 response surfaces, the §50 architecture diagram, the §59 inspector) and *steered* (the §59.2 verbs, the §59.3 manual mode) — but never *what-iffed*. This round adds four pure derivations over data that already exists — the `experiments.jsonl` entries (16 / 25.4), the `SPEC_FIELDS` registry (36.2), and the §57 per-field stats — with **zero retraining** and zero new logged fields (G2). All four are the counterfactual machinery of `simulate.what_if` (40.2) made interactive: a **live what-if preview** that re-renders the would-be candidate and its estimated effect off the logged surface (60.1); a **spec fingerprint** ("DNA") that makes two specs eyeball-comparable (60.2); an **interaction heatmap** of co-mutated field pairs (60.3); and **objective-weight sliders** that re-slice which logged candidates would pass (60.4). The app is a thin renderer over the new `whatif` core module (60.5); nothing under defaults changes (A1–A49 green).

### 60.1 The live "what-if" preview

`whatif_preview(entries, best_spec, field, value)` (`whatif.py`): given the run's logged `entries`, the current champion `best_spec` (a dict, or `None` → `DEFAULT_SPEC`, the 59.3 defaults), a registry `field`, and a candidate `value`, it returns the would-be picture — the **candidate** (`dict(best_spec)` with `field` force-set to `value`, the §59.2 pin semantics on one field), its **valid / error** (the *combination* validated against `ModelSpec`: a registry-legal value can still be a `SpecError` for the champion's family — e.g. a tree depth on an mlp — the §59.3 `manual_spec` rule), and its **estimated effect** read off the logged response surface (57.2 `field_response_stats`): `value_mean` / `value_n` for the candidate `(field, value)` pair, `current_mean` for the champion's current value of the field, and `delta = value_mean − current_mean` — each `None` when the pair was never logged (an honest "no data yet", never an invented number), plus the **pool** (the number of scored candidates the surface was built from). `field` must be in the `SPEC_FIELDS` registry (G2, 36.2 — a `ValueError` naming the registry) and `value` must pass the field's registry validator (a loud `ValueError`). Pure and deterministic (G2).

### 60.2 The spec fingerprint ("DNA")

`spec_fingerprint(spec)` / `fingerprint_diff(a, b)` (`whatif.py`): one row per registry field, in registry order (deterministic, G2), `{"name", "kind", "value", "t"}` with `t ∈ [0, 1]` the value **normalized within the field's own space** (the registry `space` — never an ad-hoc bound): **ordered** fields min-max over the space, clamped to `[0, 1]` (a single-point space → `0.0`); **categorical** fields by ordinal position, `index / (len − 1)` (a value outside the space → `None`); **sequence** fields (the architecture) by the *scale* reading — `max(numeric components) / max(all numeric components in the space)` (the "wide / narrow" axis; depth is metadata the renderer shows in the tooltip). A `None` field value → `t = None` (the field was never set — e.g. `knn_k` on an mlp — the renderer shows the value with no bar, never a fake 0). `spec` is a dict or a `ModelSpec`; `None` → `[]`. `fingerprint_diff(a, b)` compares two specs per field — `{"name", "value_a", "value_b", "t_a", "t_b", "delta"}` with `delta = t_b − t_a` (`None` when either spec leaves the field unset). Two specs on the same task become two comparable bar rows — "wide + shallow" vs "narrow + deep" at a glance.

### 60.3 The interaction heatmap

`interaction_matrix(entries)` (`whatif.py`): which field mutations **co-occurred** in the same candidate and their joint effect — the interaction the single-field views (57.2 / 57.4) hide ("hidden_dim only helps when lr is low"). The candidate pool is the 57.2 scored rule (baseline + experiment rows with a finite `holdout_score` and a dict `spec`; screen / curriculum / invalid rows never enter). A candidate's mutation set is its logged `mutation` list (26.3 — the fields that differ from the champion in force), restricted to registry fields (G2, 36.2). For every *unordered pair* `{f1, f2}` both mutated in the same candidate: `n` candidates, the `mean` holdout score, and `delta = mean − overall_mean` (the run's mean over the whole scored pool, incl. the baseline — the same credit pool as 57.2). Returns the JSON-safe dict `{"fields": [...sorted...], "cells": {f1: {f2: {"n", "mean", "delta"}}}, "marginals": {field: #candidates that mutated it}, "overall_mean", "n_scored"}`. Pure and deterministic (G2); an empty pool returns the empty shape (the renderer draws its empty state).

### 60.4 The objective-weight reslice

`weighted_reslice(entries, w_score, w_train, w_size=0.0, target=0.75, state_dim=None, n_out=None, grid=None)` (`whatif.py`): re-weight score vs train-time vs model-size and re-slice which **logged** candidates would pass — the §40.2 counterfactual re-gater, made interactive (zero retraining). The pool is the 57.2 scored rule. Each candidate gets a **composite** in `[0, 1]` — the weighted average of its *pool-relative* min-max positions: the score axis (higher better), the train axis (`1 − minmax(train_seconds)`; a missing train drops the axis for that candidate), and the size axis (`1 − minmax(spec_n_params(spec, state_dim, n_out, grid))`, 58.2; `None` for data-dependent families / missing geometry — the axis drops for that candidate). A candidate's composite renormalizes over the axes it has, so `w_size > 0` never punishes an unsized family; a degenerate pool axis (`hi == lo`) gives every candidate the same normalized position (all tied on that axis — `_minmax` `1.0`, so the lower-is-better axes read `0.0` for everyone — and the ranking is unaffected). **pass** = `composite >= target`. Validation is loud at construction time: every weight a finite number `>= 0` (at least one strictly positive), `target` a finite number in `[0, 1]` — else a `ValueError`. Returns `{"weights", "target", "pool", "passing", "pass", "final", "candidates"}` — `final` is the passing candidate with the highest composite (ties: lower `train_seconds`, then log order — the 40.2 convention), `None` when none pass; each candidate carries `cand` (1-based log position), `spec_hash`, `score`, `train`, `size`, the three normalized axes (`n_score` / `n_train` / `n_size`, `None` = axis absent), `composite`, and `pass`. Pure and deterministic (G2); an empty pool returns the zero shape (`pass: False`) without raising.

### 60.5 Surfaces + exports

- **renderers (60.5.1)** — four SVG renderers in `plotting.py`, all valid XML, deterministic (G2), palette / dark-aware, with a `<title>` tooltip per datum: `svg_whatif_effect` (the two bars — the champion's current value vs the what-if value — on a 0–100 clamped scale, the signed delta annotated above; a never-logged value renders an honest empty state, not a fake zero); `svg_spec_fingerprint` (one row per fingerprint field — a single bar for one spec, a paired top/bottom bar pair for A vs B — the value labels on the right); `svg_interaction_heatmap` (the field × field grid — off-diagonal cells filled by `delta` (green lift / red drag, opacity ∝ `|delta| / max|delta|`) with the `n` count inside, the diagonal the per-field marginal, never-co-mutated pairs blank); `svg_weighted_reslice` (the composite 0–1 axis with the dashed target line, one dot per logged candidate at its composite colored pass/miss, a `★` over the counterfactual final, the weights in the header caption).
- **app (60.5.2)** — `dashboard_app.py` renders the block in `_render_result` (after the §59.1 parameter inspector, before the learning views): a `What-if & comparison (SPEC.md 60)` subheader with (i) the what-if field / value selectboxes (the value options are the field's registry space) → the would-be candidate (the §50 `svg_architecture` with the field highlighted) beside the estimated effect (60.1), (ii) the fingerprint with an optional champion-vs-baseline compare (60.2), (iii) the interaction heatmap (60.3), and (iv) the four sliders — score (default 0.5), train-time (0.25), model-size (0.25), composite target (0.75) — → the reslice chart + a success (passing / pool + the counterfactual final candidate) or a warning (no candidate passes) (60.4). Display-only (AppTest-safe); every failure path is a friendly caption (the 49.4.3 pattern), never a page crash.
- **exports (60.5.3)** — the five new `whatif` names (`whatif_preview`, `spec_fingerprint`, `fingerprint_diff`, `interaction_matrix`, `weighted_reslice`) and the four new renderer names (`svg_whatif_effect`, `svg_spec_fingerprint`, `svg_interaction_heatmap`, `svg_weighted_reslice`) are in `__all__` (33.1).

### 60.6 Acceptance (A50)

- **60.1 core** — `whatif_preview` is hand-computed: an unknown `field` is a `ValueError` naming the registry; a bad `value` is a `ValueError` from the field's validator; over a baseline + scored-experiment fixture the `candidate` dict, `valid` / `error`, the per-value `value_mean` / `value_n` (the 57.2 means), the champion's `current_mean`, `delta`, and the `pool` are exact; a never-logged value → `value_mean` / `delta` `None` (honest, not zero); an invalid combination for the champion's family (e.g. a tree depth on an mlp) → `valid: False` with the `SpecError` text; `best_spec = None` → the `DEFAULT_SPEC` base (59.3).
- **60.2 core** — `spec_fingerprint` is hand-computed: one row per registry field in registry order; the `t` values exact (categorical ordinal, ordered min-max, the architecture scale reading); `knn_k` present in every row (unset on an mlp → `t` `None`); `fingerprint_diff` exact deltas + the one-side-unset → `delta` `None` case; `spec_fingerprint(None)` → `[]`.
- **60.3 core** — `interaction_matrix` is hand-computed: the fixture's `overall_mean` (incl. the baseline), the co-mutated pair's `n` / `mean` / `delta`, the marginals, the `fields` order; non-registry names in a logged `mutation` are filtered out; screen / curriculum kinds and non-dict specs are excluded; empty `entries` → the empty shape.
- **60.4 core** — `weighted_reslice` is hand-computed: the fixture's three composites (the pool-relative min-max axes, the unsized family's renormalized composite, the degenerate-axis `1.0`), the pass set at the target, and `final` (the composite tie broken by train time, then log order — 40.2); zero weights / a negative weight / a non-finite weight → `ValueError`; a `target` outside `[0, 1]` → `ValueError`; an empty pool → the zero shape (`pass: False`), no exception.
- **renderers (60.5.1)** — all four renderers are valid XML with the key content tokens (the signed delta, the `n=` counts, the target label, the `★` final marker, the `<title>` tooltips); the empty states (a never-logged value, no co-mutated fields, an empty pool) render without raising; the `okabe` and dark re-renders differ; a bad `palette` is a `ValueError`.
- **app (60.5.2)** — the app's source wires the block: the `What-if & comparison (SPEC.md 60)` subheader + the `wf_field` / `wf_value` / `fp_compare` / `wobj_score` / `wobj_train` / `wobj_size` / `wobj_target` keys + the `whatif_preview` / `interaction_matrix` / `weighted_reslice` calls + the four renderer calls; one light AppTest renders the block with no exception.
- **exports (60.5.3)** — the five new `whatif` names and the four new renderer names are in `__all__` (33.1).
- **version + regression** — the version steps to `0.46.0` in both sources (33.1); A1–A49 stay green (no default-path behavior change; the report / share-bundle byte-identity pins untouched); the A25 index advances (46 acceptance rows; `defined == set(range(1, 51))`).

### 60.7 Milestone (M49)

**M49** — v0.46 "C. Parameters: visually interpreted and modified (part 2): what-if & comparison": the live what-if preview over the logged surface (60.1), the spec fingerprint "DNA" (60.2), the interaction heatmap of co-mutated fields (60.3), and the objective-weight reslice — the §40.2 counterfactual re-gater made interactive (60.4) — all pure derivations over the run's data, zero retraining (60.5), byte-identical under defaults (G2) (A50).

## 61. More specialized scenarios (v0.47)

The improver has proven it can find a *score*, but a real deployment usually needs a score **subject to** other constraints (per-class fairness, calibration, cost, monotonicity), a **domain** with its own data shape and bar (medical, finance, robustness), and the ability to **stress** the search (few-shot, drifting, trap spaces). This round adds all five as **additive, opt-in** surface over machinery that already exists — the §37 objective set (`gate.py`), the §36 Task ABC, the §21 plugin loader, and the §18 gate — with **no default-path behavior change** (A1–A50 green). The through-line: *the gate already evaluates a set of named `name op threshold` objectives; A1 generalizes the name registry and the `actuals` producer; A2–A5 are the specialized tasks/scenarios that populate those names.*

### 61.1 A1 — Constraint-aware search

`gate.py` already evaluates `evaluate(objectives, actuals)`: **PASS iff ALL** `name op threshold` objectives pass, a missing actual failing its objective. A1 generalizes the **name registry** from `(score, train, model)` to also accept the constraint metrics: `per_class_f1`, `ece`, `cost`, `monotonic_in`. Each is the same `name op threshold` form (e.g. `per_class_f1>=0.9`, `ece<=0.05`, `cost<=3.0`, `monotonic_in>=1.0`) — a constraint is a *lower bound on a goodness* or an *upper bound on a badness*, so it is an `Objective`, not a new type. The `improver`'s reward becomes *score subject to constraints* because the acceptance gate (37.2) is where PASS/FAIL is decided; the loop is untouched.

- **`OBJECTIVE_NAMES`** gains the four names (61.1.1). `parse_objective` / `default_objectives` / `evaluate` are otherwise unchanged — **`default_objectives(target)` is byte-identical** (the A1–A50 pin).
- **`compute_actuals(env, task, model, entries, extras=None)`** (61.1.2): the richer `actuals` producer. It starts from `actuals_from_run` (score/train/model) and, when `task` is given, adds the constraint actuals by reusing existing primitives: `per_class_f1` from the confusion matrix in `diagnostics.holdout_diagnostics` (61.1.3), `ece` from the model's holdout softmax probabilities (`calibration.ece`, 61.4), `cost` from the task's per-episode cost (`task.cost(model, split, n)`, 61.5), `monotonic_in` as a finite-difference slope sign over a held-out row (`calibration.monotonicity_slope`, 61.1.3). `extras` (a dict) can supply a precomputed value for any name (e.g. the app passes an ECE it already computed) — a supplied value wins over recomputation. Any name with no actual stays `None` → its objective fails (the 37.2 rule), never a silent pass.
- **`calibration.py`** (new, 61.1.3): `per_class_f1(confusion)` (per-class F1 from a confusion matrix; the *minimum* over non-empty classes is the `per_class_f1` actual — a fairness reading, "the worst class"), `ece(probs, labels, n_bins=10)` (expected calibration error, 61.4), `coverage_width(pred_lo, pred_hi, labels)` (fraction of labels inside their predicted interval, 61.4), and `monotonicity_slope(fn, x0, dx, k=3)` (a central finite-difference slope sign estimate — `>= 0` when the function is (locally) non-decreasing). All pure numpy, deterministic (G2).

### 61.2 A2 — Domain task packs

Three reference `Task` subclasses in `tasks/` (delivered through the §21 plugin loader — the in-repo `TASKS` registry is the same mechanism an external entry-point uses, so a downstream package could re-supply any of them unchanged). Each is a **`Task`** subclass, deterministic given the seed (G2), with a `name`, `metric`, `capabilities`, and (for A1) a `holdout_rows` protocol method so the constraint actuals are computable:

- **`MedicalTabularTask`** (61.2.1) — `medical-v1`. An *imbalanced*, *cost-sensitive* binary classification: ~5% positive by construction; `head = "softmax"`, `metric = "cost_sensitive"`, `score = 100 · (TPR − β·FPR)` with `β` a tunable false-positive penalty (default `1.0`), clamped at 0 — a single number the improver can climb while the *per-class* constraint (A1) keeps the minority class from being sacrificed. Exposes `holdout_rows`, `class_values = [0.0, 1.0]`, and a `starter_spec` (a `DEFAULT_SPEC`-shaped dict, the domain-appropriate starting point) and `default_objectives` (the domain's bar: `score>=target` plus the domain's constraint set).
- **`FinanceForecastTask`** (61.2.2) — `finance-v1`. A *time-ordered* forecasting task: a seeded random-walk-plus-sinusoid price series, `split_mode = "temporal"` (walk-forward — the §45.1 rule, no leakage), predict the **next-`h`** value (horizon `h`, default 1); `head = "mse"`, `metric = "directional"`, `score = 100 · directional_accuracy` (fraction of steps where the sign of the predicted change matches the true change; a flat prediction → 50). Exposes `holdout_rows` (the standardized (x, y) horizon pairs) for A1's constraint actuals (here `cost` = mean absolute scaled error, 61.5) and `starter_spec` / `default_objectives`.
- **`RobustTask`** (61.2.3) — `robust-v1`. A *wrapper* over a base fitting task that scores on a **shifted** holdout: `score = worst-group accuracy` (the minimum, over `n_shift` perturbation levels of seeded Gaussian noise σ, of the base task's per-group accuracy on that level's holdout). It delegates `make_dataset` / `prepare` to the wrapped task but *overrides `score`* to take the worst group — the robustness/OOD bar. `head` / `n_outputs` / `state_dim` mirror the wrapped task; it exposes `holdout_rows` (the cleanest perturbation level's rows) and `starter_spec` / `default_objectives`. The base task is given at construction (a `CsvTask` or a synthetic task), so this is a *curated mutation space*: the improver searches the base task's spec space, but the acceptance bar is the worst group.

All three register in `TASKS` (additive, 61.2.4) and update `tasks/__init__.py` `__all__`.

### 61.3 A3 — Regime / stress scenarios

A `Scenario` dataclass (`scenarios.py`) — a **pure config** that composes with any task + budget + gate (the core loop is unchanged): `name`, `description`, `task` (a `Task` instance or factory), `budget_overrides` (a `dict` of `Budget` fields), `gate_overrides` (a `dict` of gate fields, e.g. a tighter `z` / `gen_gap`), and `extras` (a `dict`, e.g. A1's constraint `objectives`). Three presets ship:

- **`fewshot`** (61.3.1) — a tiny train pool (small `n_points` via the task's `default_dataset_size` override) against a full holdout — does the CI / gen-gap gate (18.5) catch an overfit candidate?
- **`drifting`** (61.3.2) — a `ParityTask`-like task whose observation noise (`p_flip`) is *raised* at the gate step — the improver must re-adapt; composes with the §20.1 curriculum (the difficulty ladder already exists; this pins one level).
- **`trap`** (61.3.3) — a spec field that looks good on val but hurts holdout — a gate that the §18.5 gen-gap penalty exists to catch; the preset sets a tight `gen_gap` threshold so a val-holdout divergent candidate is rejected.

Each preset is `Scenario(...)`-constructible and returns a resolved `Budget` / gate config (61.3.4); applying a preset to a task is pure (no training, no env state).

### 61.4 A4 — Uncertainty / calibration

ECE (expected calibration error) becomes a first-class gate name (61.1) computed over the model's **softmax probabilities** on the holdout — a post-hoc metric, **no retraining**, no new head. `calibration.ece(probs, labels, n_bins=10)` bins by predicted max-probability, computes `Σ_bins (n_bin/n)·|acc_bin − conf_bin|` in `[0, 1]` (a perfectly calibrated model → ≈ 0; a uniform or confident-wrong model → high). `calibration.coverage_width(pred_lo, pred_hi, labels)` is the interval-coverage companion (fraction of labels inside `[pred_lo, pred_hi]`), the natural objective for an *interval* predictor (61.4.1). The **ensemble-spread** predictor (61.4.2) — average the top-`k` frontier specs' holdout probabilities before scoring ECE — reuses the §37.1 Pareto frontier (`frontier()`) and is a pure function `ensemble_ece(task, models, k=3)` in `calibration.py` (opt-in; the app / a future `fit` flag can wire it, the core only provides the primitive).

### 61.5 A5 — Resource-constrained control

`cost` becomes a first-class gate name (61.1). For **interactive** tasks the cost is the sum of per-step action magnitudes over an episode (a task-level `cost(model, split, n)` — the episode primitives `act`/`step_vec` already exist); for **fitting** tasks (61.2.2) it is a domain-specific per-prediction badness (e.g. mean absolute scaled error). The improver balances `score` vs `cost` — a new objective in the gate (37.2), **not** a new loop. `ParetoFrontier` (37.1) is *optionally* extended: a `cost_of(spec_hash)` accessor reads a `cost` map (61.5.1) so the frontier summary can report a `(score, train, cost)` triple, but the frontier's default `(score, train)` contract is byte-identical (the cost objective can be satisfied entirely in the gate without touching the frontier — 61.5.2).

### 61.6 Acceptance (A51)

- **61.1 core** — `parse_objective` accepts the four new names (a `per_class_f1>=0.9` / `ece<=0.05` / `cost<=3.0` / `monotonic_in>=1.0` each parse to the right `Objective`); an unknown name is still a `ValueError`; **`default_objectives(target)` is byte-identical** (the A1–A50 pin); `evaluate` over a richer `actuals` dict (all four constraint names present) is PASS iff ALL pass, and a missing constraint actual fails its objective; `compute_actuals` threads `extras` over recomputation and leaves unknown names `None`.
- **`calibration.py`** — `per_class_f1` is hand-computed (the min over non-empty classes; an empty class is excluded, never a `0.0` that sinks the min); `ece` ∈ `[0, 1]`, a perfectly calibrated model → ≈ 0, a confident-wrong model → high, and it is deterministic; `coverage_width` ∈ `[0, 1]`; `monotonicity_slope` is `>= 0` on a non-decreasing function and `< 0` on a non-increasing one; `ensemble_ece` is deterministic and `<=` the worst single model's ECE on a hand-built fixture.
- **61.2 tasks** — each new task's identity (`name`, `state_dim`, `n_outputs`, `head`, `metric`, `capabilities`, `split_mode`) is exact; `make_dataset` returns the right shape; `score` is in `[0, 100]` (clamped) and **deterministic** (same seed → same score, G2); `holdout_rows` is present (for A1's `per_class_f1` / `cost`); `medical` is imbalanced (positive rate ≈ the declared rate) and `finance` is temporal (train precedes holdout, no row reuse); each is registered in `TASKS`.
- **61.3 scenarios** — each preset is a `Scenario` with a `name`, a `description`, a `task` (a `Task` instance), `budget_overrides`, `gate_overrides`; `fewshot` yields a small train pool; `drifting` raises the base task's noise at the gate level; `trap` sets a tight `gen_gap`; a preset composes with any `Budget` / gate (pure, no env).
- **61.4 A4** — `ece` / `coverage_width` bounds + determinism; the ensemble-spread predictor (`ensemble_ece`) is a valid, deterministic ECE over averaged top-`k` probabilities.
- **61.5 A5** — the `cost` name is in the gate registry; a low-cost model passes `cost<=X` and a high-cost model fails; the optional `ParetoFrontier.cost_of` accessor returns the mapped value and leaves the default `(score, train)` contract byte-identical.
- **exports (61.6.5)** — the new top-level names (`per_class_f1`, `ece`, `coverage_width`, `monotonicity_slope`, `ensemble_ece`, `compute_actuals`, `MedicalTabularTask`, `FinanceForecastTask`, `RobustTask`, `Scenario`, `fewshot`, `drifting`, `trap`) are in `__all__` (33.1).
- **version + regression** — the version steps to `0.47.0` in both sources (33.1); A1–A50 stay green (no default-path behavior change; `default_objectives` byte-identical; the report / share-bundle byte-identity pins untouched); the A25 index advances (47 acceptance rows; `defined == set(range(1, 52))`).
...
## 68. The RL loop in the dashboard (v0.54)

The RL path was CLI-only by design (SPEC.md 23.1): the dashboard's `DashboardRunner` refused `policy='rl'` ("it trains over several full-budget episodes"), and the app's only RL surface was the precomputed policy-report view (D2, SPEC.md 29.2) — *never running RL live*. v0.53 (67) made the policy durable and steerable, but the self-improving cycle still had no UI home: no place to see the episodes tick by, no place to pick up a saved policy and keep improving it, no place to download the result. This round closes it: the RL loop runs in the app, with the v0.53 features (persistence, ε-greedy, best-episode checkpoint, multi-task transfer) as first-class UI verbs.

The through-line: *the worker protocol is already policy-agnostic — give the RL loop the same runner interface.* The app's worker/drain (`_worker_loop` / `_drain_live`) only ever calls `start()` → `next()`…→ `finish()` and renders the posted messages (51.2.3). `DashboardRunner` is one implementation of that contract; this round adds `RLRunner` and `RLMultiRunner` (68.1) in a new streamlit-free core module (`rl_dashboard.py`, the 23.1 pattern), so the app stays a thin renderer, the core stays testable without streamlit, and the bandit/search app path is byte-identical (68.4, the A58 pin anchor).

### 68.1 The RL runners (R1)

**`RLRunner`** (68.1.1) — `start()` / `next()` / `done` / `finish()` / `request_stop()` with the same message shapes the drain already renders: the `info` dict carries the same keys (label, head, rows, baseline, target, seed, budget, run_dir) plus the RL knobs (`episodes`, `epsilon`, `epsilon_decay`, `n_updates`, `resumed`); each `next()` returns the same update dict (index, accepted, reason, candidate_score, gen_gap, train_seconds, experiments_left, mutation, spec_diff, loss_history, best_score, reward, …) plus the RL extras (`episode`, `epsilon`, `episode_return`, `episode_done`, `task`). The loop body is exactly `train_policy`'s (67.x): `env.reset()` per episode, then `propose` → `step` → `observe` until the episode's `done`; the difference is granularity — `next()` yields one step at a time so the app can render live, and a stop request (51.2.2, honored by the env's `stop_check` between experiments) ends the run honestly as a partial (the in-flight episode's gradient is flushed by the env's `done` signal, the same as the CLI).

**One-home loop semantics** (68.1.2) — `RLRunner` and `train_policy` are the same loop: cross-pinned by a test — for the same (task, seed, budget, N episodes), the per-episode returns of `RLRunner` and of `train_policy` driven over an identical env/policy are identical. One of them changing without the other is a suite failure (the 23.1 "run semantics live in the core" guarantee, now covering RL).

**Keep-best = best-checkpoint download** (68.1.3) — with `keep_best=True` (the app's default), `finish()` swaps the policy's weights to the **strictly-greater** best-episode snapshot (the 67.3.1 rule) before serializing, so the downloaded npz **is** the best checkpoint; `keep_best=False` (the CLI parity default) downloads the final episode's weights. `finish()`'s result dict carries the shared scalar keys the result view needs (verdict vs target, final/baseline scores, improvement factor, experiments, wall time, finished_reason, best_spec) plus an `rl` block: `{episodes, episode_returns, best (or None), n_updates, epsilon_final, trace, returns, task_names, policy_bytes, keep_best, resumed}`.

**`RLMultiRunner`** (68.1.4) — multi-task transfer in the app (20.2): one shared `MetaRLPolicy(task_names=…)`, round-robin `ep % n_envs` over the built-in tasks (parity-v1 / sine-v1 / cartpole-v1), `episodes_per_task` episodes each — the exact `train_multi_policy` order (67.3.2's ambiguity is why multi has no keep-best: `best` stays `None` there). Same `start` / `next` / `done` / `finish` interface, so the Compare tab can drive it live; each update carries its `task`.

### 68.2 Policy in & out (R2)

**One home for the policy bytes** (68.2.1) — `improver/rl_policy.py` gains `policy_to_bytes(policy) -> bytes` and `policy_from_bytes(data, seed=0) -> MetaRLPolicy` (npz in memory, the same arrays and the same `FORMAT_TAG` — file and bytes formats cannot drift), and `save_policy` / `load_policy` are refactored onto them (the 67.1 round-trip pins stay green: the file format is unchanged). `policy_from_bytes` carries the 67.1.2 validation (tag, shape, `B`/`task_names` consistency) over arbitrary bytes: garbage in is a `ValueError`.

**In the app** (68.2.2) — the RL options take an initial policy (an npz upload or path): the app previews it with `describe_policy(policy)` — `{n_actions, state_dim, task_names, n_updates, epsilon, epsilon_decay, lr, baseline_decay, has_bias}`, a JSON-safe dict for the preview card — then `RLRunner` resumes from it (67.1.3's deployment property, UI-side: a saved policy can be loaded and *continued* in the app). **Out of the app** — the result view offers `Download the trained policy (.npz)` carrying the `rl` block's `policy_bytes` (the keep-best swap, 68.1.3) — the learned improver is a deliverable artifact next to the learned model.

### 68.3 The UI surface (R3)

- **Sidebar** (68.3.1) — the policy selectbox gains `rl`; the "RL options" block (episodes, ε, ε-decay, keep-best, initial-policy npz upload + `describe_policy` preview card) renders when the policy is `rl`, following the 48.1.2 pattern: widgets render only under "Show advanced options", and when that toggle is off the defaults (`rl_episodes=5`, `rl_epsilon=0.0`, `rl_epsilon_decay=1.0`, `rl_keep_best=True`) are materialized into session state so the values are always defined and presets/AppTests can set them without the widgets present. A new app module `rl_dashboard` joins the app-language scan (35.2): its user-facing strings are as clean as the other app modules.
- **Live loop** (68.3.2) — `_run` builds an `RLRunner` when the policy is `rl`; the worker/drain protocol is otherwise untouched. One drain addition: when an update carries `episode_done`, an episode-boundary line renders (episode k of N · return · current ε) — the table, curve, and decision views keep rendering per step as they do for bandit.
- **Result block** (68.3.3) — when the result carries an `rl` block: the episode-returns curve (`svg_task_returns`), the best-episode card (68.1.3), the policy download (68.2.2), and the live policy views over the stored D2 trace — `svg_action_probabilities` (top-k actions over the sampled steps) + `svg_policy_trace` — the D2 view (29.2) rendered from a run that just happened, not a precomputed directory. The precomputed policy-report panel stays (both paths valid; the CLI remains a surface too, 67.4).
- **Compare tab** (68.3.4) — a live multi-task panel: a task multiselect over the built-ins, episodes per task, one press drives `RLMultiRunner` synchronously and renders the per-task return curves, the shared-policy download, and the `describe_policy` card.

### 68.4 The default path is untouched (the A58 pin anchor)

A1–A57 stay green because every v0.54 addition is additive or opt-in: `RLRunner` / `RLMultiRunner` / `describe_policy` are new classes no existing caller invokes; `policy_to_bytes` / `policy_from_bytes` are new functions and the refactored `save_policy` / `load_policy` write byte-identical files (same arrays, same `FORMAT_TAG`, 67.1); the app's new widgets exist only under `policy == "rl"` (default `bandit`), with the 48.1.2 defaults materialized when the advanced toggle is off, so a bandit/search app run renders byte-identically to pre-v0.54 (55.4's default-path guarantee, re-asserted); the drain's one new branch fires only on `episode_done`, which bandit/search updates never carry (68.3.2); A6's RL pins, A57's self-improving pins, and every app-language scan pass unmodified (A1–A58 green).

### 68.5 Acceptance (A58)

- **68.1 lifecycle** — `RLRunner` on a tiny built-in task (2 episodes, budget 1–2): `start()`'s info carries the shared keys + `episodes` / `epsilon` / `epsilon_decay` / `n_updates` / `resumed`; each update carries `episode` / `epsilon`; `done` after exactly `episodes` episodes; `finish()`'s verdict/final/baseline keys are present, the `rl` block's `episode_returns` has length `episodes`, the trace is non-empty, and `policy_bytes` decodes via `policy_from_bytes` with `n_updates == episodes`.
- **68.1.2 one-home cross-pin** — `RLRunner`'s per-episode returns equal `train_policy`'s on the same (task, seed, budget, N episodes) with identically seeded envs/policies — the loops are the same loop.
- **68.2 round-trip** — `policy_from_bytes(policy_to_bytes(p))` is bit-equal to `p` (w/b/B/hyper-parameters/n_updates/ε schedule), complementing A57's file-based pin; garbage bytes ⇒ `ValueError`; `episodes=0` ⇒ `ValueError`.
- **68.2 resume** — `start()` with `initial_policy` bytes yields a policy whose weights and `n_updates` equal the saved policy's; `epsilon=1.0, decay=0.5` over N episodes ⇒ the updates' ε values follow `0.5·0.5ᵏ` (67.2.2, UI-side).
- **68.1.3 keep-best** — with `keep_best=True` the decoded `policy_bytes` weights equal the end-of-best-episode weights (replayed independently); `keep_best=False` equals the final episode's.
- **68.1.4 multi-task** — `RLMultiRunner` (two built-in tasks, 1 episode each): done; the `rl` block's `returns` has both tasks; the decoded policy has `B is not None` and `task_names == tasks`; the update `task` sequence starts with `tasks[0]` (round-robin).
- **68.3 app** — AppTest (streamlit optional-skipped): with `policy="rl"` a tiny run completes without exception and the result view exposes the policy download button + the episode/policy-view markdown; an idle boot with policy `rl` does not raise (68.3.1–68.3.3).
- **68.4 pin** — A57's suite (`tests/test_rl_selfimprove_v053.py`) and A6's (`tests/test_rl_policy.py`) pass unmodified; the app-language scan covers the new `rl_dashboard` module and passes (68.3.1).
- **exports + version** — `RLRunner` / `RLMultiRunner` / `describe_policy` / `policy_to_bytes` / `policy_from_bytes` are in `autorefine.__all__` (33.1); the version steps to `0.54.0` in both sources (33.1); the A25 index advances (54 acceptance rows; `defined == set(range(1, 59))`).
...
## 75. Modeling capability v0.61 — `adamw` optimizer, three LR schedules, GP and GAM families (v0.61)

Directive: *"Increase the modelling capabilities so it would be more useful for research and application purposes."* Four additive, **value-level** model-space extensions (no new `ModelSpec` fields — the 15-field registry, the A24/A26 pins, and the bandit draw-space stay byte-identical): an `adamw` optimizer with decoupled weight decay (75.2.1), three LR schedules `step`/`exponential`/`cyclic` (75.2.2), and two non-parametric families — a Gaussian process with fixed-seed random Fourier features (75.2.3) and a generalized additive model of smoothing splines (75.2.4). The new *values* are reachable everywhere the spec surface already reaches (config validation, trainer dispatch, the 83-action RL/Gym catalog, `eval`/`predict`/`fit` loaders, the spec chips, the architecture diagram); the **bandit** keeps drawing the legacy value sets so the A1–A4 proposal stream and the §18.7 bit-exact sequence are untouched (75.2.5) — the same legacy-stability precedent as `knn`/`convnet` in §25.

### 75.1 Problem

- **The neural path had no decoupled-decay optimizer.** `weight_decay` was folded into the gradient (L2) for every optimizer, which couples decay to the adaptive scale — the classic Adam-with-L2 pathologies on small overfit-prone tasks. Research users expect AdamW as a first-class value.
- **The schedule set stopped at warmup/cosine.** `step`, `exponential`, and `cyclic` are the standard remaining catalog (Determined/W&B-style) and each is a pure deterministic function of `(t, T)` — zero new machinery in the trainer loop.
- **The family set was parametric-only** (mlp/tree/boost/knn/convnet). Two canonical non-parametric research families were missing: **GP** (native kernel regression, strong on small noisy data, the sine/parity ceiling) and **GAM** (interpretable additive curves — the "application" side: a per-feature effect the domain reader can inspect). Both fit the existing `fit`/`forward`/`save`/`load` contract (the KNN template, §25.2), so scoring, ensemble, `eval`, `predict`, and the dashboard work unchanged.

### 75.2 Design (value-level, pin-safe)

- **75.2.1 `adamw` optimizer** — `AdamW` in `models/optimizers.py`: identical moments to `Adam`, with the decay **decoupled** — applied to the *parameters*, not the gradient: `param -= lr·m̂/(√v̂+ε)` (exactly the Adam update, same op order) then `param -= lr·wd·param`. `weight_decay` is consumed at construction via `make_optimizer(name, lr, weight_decay=0.0)` (backward-compatible default); the trainer skips the shared gradient-L2 decay for `adamw` (no double decay) and passes `wd=0.0` for every other optimizer, so all legacy paths are bit-identical. **Value identity:** `wd == 0.0` gives exactly the `Adam` update (bit-identical — pinned by A65), so a `weight_decay == 0` adamw run equals the adam run.
- **75.2.2 three LR schedules** — `_scheduled_lr` gains `step` (plateau by a fixed ratio at a fraction of the budget), `exponential` (geometric decay to a floor), and `cyclic` (triangular up/down between a floor and the base LR) — pure, deterministic, seeded-identical; the legacy three branches are untouched (A60's constant-schedule value identity and the §18.7 stream stay green).
- **75.2.3 `gp` family** — `models/gp.py`: one-vs-rest ridge regression on a **fixed-seed** (seed 0) random Fourier feature map (`_N_FEATURES=128`, `_SIGNAL_VAR=1.0`, `_NOISE=1e-3`): `w = solve(ΦᵀΦ + noise·I, ΦᵀT)`; softmax = one-vs-rest means as logits, mse = the single regression. Deterministic and independent of the training seed (like KNN — documented); O(n·m·d) queries, no O(n³) kernel solve (a deliberate approximation for the v1 dataset sizes).
- **75.2.4 `gam` family** — `models/gam.py`: `ŷ = α + Σⱼ fⱼ(xⱼ)` with a natural-cubic (truncated-power) basis per feature (`k = min(6, max(1, n//8))` interior knots at data quantiles); the smoothing penalty is the **curvature** of each fitted per-feature curve — `√weight_decay · (D @ Nⱼ)` with `D` the second-difference operator — so the spec's `weight_decay` *is* the smoothness knob (0.0 = interpolating spline); solved per output unit with `np.linalg.lstsq` (no RNG). Interpretable by construction: each `fⱼ` is a single-variable curve.
- **75.2.5 bandit draw-space stays legacy** — the load-bearing pin guard: `FIELD_SAMPLERS["optimizer"]` draws the legacy 3-value tuple `("sgd","momentum","adam")` (same order as the pre-v0.61 3-element `OPTIMIZERS` — RNG consumption unchanged) and `FIELD_SAMPLERS["lr_schedule"]` the legacy `("constant","cosine","warmup_cosine")`; `relevant_families` is untouched (3 flat / 5 grid), so the bandit never offers `gp`/`gam` — the §18.7 bit-exact sequence and A1–A4 stay green. The new values are offered where they belong: the 83-action catalog (A26 re-derived 77→83; A8 37→39 tree actions), the spec surface, `mutate_spec_dict`, the RL/Gym adapter, and `fit`/`predict`/`eval` loaders. `_coerce_for_family` and `apply_action` gain a `gp`/`gam` no-architecture branch (they ignore `architecture`, like knn); `spec_n_params` reports `None` (data-dependent, like tree/boost/knn); the architecture diagram and the exec `in-words` render proper gp/gam blocks instead of "unknown family".

### 75.3 Evidence

- **75.3.1 capability smoke** — on `sine-v1` (300 points, mse head): gp loss **0.639**, gam loss **0.046** vs the constant baseline **1.561** (both beat it by wide margins); `GP`/`GAM` `save`/`load` round-trips are forward-identical (`npz`, `allow_pickle=False`); an `mlp` with `optimizer=adamw` + each new schedule trains to a finite loss; `adamw(wd=0)` is bit-identical to `adam` on a fixed training (the value identity); `train_from_task` runs gp and gam end-to-end on a real task (scratchpad `smoke_v061.py`).
- **75.3.2 the pins** — `test_legacy_regression_sequence_bit_exact` (the §18.7 stream), A26 (re-derived: 83 actions, 15 fields, `model_family` = all 7 families), A8 (re-derived: 83 actions, 39 tree actions), A24, A50's fingerprint (re-derived: momentum ordinal 1/3 in the 4-value space), A60's bit battery, the RL/Gym catalogs (self-consistent via `len(ACTIONS)`), and the coherency A-index — all green (75.5).
- **75.3.3 bugs caught and fixed before shipping** — (a) the first `AdamW` formulation `lr·(m̂/(√v̂+ε) + wd·θ)` was **not** bit-identical to Adam at `wd=0` (float re-association: `lr·x` vs `(lr·m̂)/d`); restructured so the decay term is added *after* the adaptive step and vanishes at `wd=0` (75.2.1) — the smoke test's `np.array_equal` caught it; (b) `GAM`'s per-feature knot list was off-by-one (`[None]` seed element → `forward` iterated a 0-d array) and re-fit accumulated knots; now `knots = []` + reset in `fit` (idempotent).

### 75.4 Deliberately not changed

- **MLP dropout / batchnorm / residual** — entangled with the analytic-backprop `loss_and_grads` cache; high pin risk against modest v1 value (v0.62 follow-up).
- **`predict --ensemble/--calibrate`** — CLI/contract surface for the GP's native predictive variance; the GP already exposes calibrated one-vs-rest means (partial application win). v0.62 follow-up.
- **New `ModelSpec` fields** — the value-level approach keeps the 15-field registry and the bandit draw-space byte-identical (75.2.5); a field-registry growth is a separate, larger pin re-derivation (the G2 machinery exists for it).

### 75.5 Acceptance (A65)

- **75.2.1** — `AdamW.step` with `wd=0` equals `Adam.step` bit-for-bit (parameters **and** state) across multiple steps; with `wd>0` the decay term equals `lr·wd·param` on the parameters (the decoupled formula); `make_optimizer("adamw", lr, wd)` consumes the decay and `make_optimizer("adam", lr)` ignores it (2-arg call unchanged).
- **75.2.2** — each new schedule is pure/deterministic and hits its documented shape: `step` plateaus (constant after the step point), `exponential` decays geometrically to its floor, `cyclic` oscillates between floor and base; all finite for every `t ∈ [1, T]` and `T > 0`.
- **75.2.3/75.2.4** — `gp` and `gam` train + `forward` deterministically on a fixed dataset (two fresh fits, `np.array_equal`); `save`/`load` round-trips are forward-identical; both beat the constant baseline on `sine-v1`; a `gp`/`gam` spec is `validate()`-legal and trainable end-to-end via `train_from_task`.
- **75.2.5 pins** — the §18.7 legacy bit-exact sequence passes unmodified; A26 re-derived (`len(ACTIONS) == 83`, 15 fields, `model_family.families == all 7`); A8 re-derived (83 actions, 39 tree actions); A50 fingerprint re-derived (optimizer ordinal in the 4-value space).
- **33.1 / A25 index** — the version steps to `0.61.0` in both sources; the A-index advances (`defined == set(range(1, 66))`, 61 acceptance rows, M64 resolving to `tests/test_modeling_v061.py`); the new `tests/test_modeling_v061.py` cites **A65** and pins `"0.61.0"`.
