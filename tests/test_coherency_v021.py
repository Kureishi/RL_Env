...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 56)), sorted(defined)  # A1..A55 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    assert rows == 51, f"expected 51 acceptance rows (M4–M54), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.51 ⇒ `0.51.0`, M54, SPEC.md 65)."""
