...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 68)), sorted(defined)  # A1..A67 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (63 acceptance rows: M4–M66.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 63, f"expected 63 acceptance rows (M4–M66), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.63 ⇒ `0.63.0`, M66, SPEC.md 77)."""
