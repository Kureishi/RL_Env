...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 62)), sorted(defined)  # A1..A61 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (57 acceptance rows: M4–M60.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 57, f"expected 57 acceptance rows (M4–M60), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.57 ⇒ `0.57.0`, M60, SPEC.md 71)."""
