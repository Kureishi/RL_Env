...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 65)), sorted(defined)  # A1..A64 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (60 acceptance rows: M4–M63.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 60, f"expected 60 acceptance rows (M4–M63), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.60 ⇒ `0.60.0`, M63, SPEC.md 74)."""
