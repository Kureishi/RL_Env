...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 60)), sorted(defined)  # A1..A59 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (55 acceptance rows: M4–M58.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 55, f"expected 55 acceptance rows (M4–M58), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.55 ⇒ `0.55.0`, M58, SPEC.md 69)."""
