...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 59)), sorted(defined)  # A1..A58 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (54 acceptance rows: M4–M57.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 54, f"expected 54 acceptance rows (M4–M57), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.54 ⇒ `0.54.0`, M57, SPEC.md 68)."""
