...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 38)), sorted(defined)  # A1..A37 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (33 acceptance rows: M4–M36.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 33, f"expected 33 acceptance rows (M4–M36), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.33 ⇒ `0.33.0`, M36, SPEC.md 47)."""
