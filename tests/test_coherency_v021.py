...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 35)), sorted(defined)  # A1..A34 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (30 acceptance rows: M4–M33.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 30, f"expected 30 acceptance rows (M4–M33), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.30 ⇒ `0.30.0`, M33, SPEC.md 44)."""
