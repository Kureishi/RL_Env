...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 66)), sorted(defined)  # A1..A65 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (61 acceptance rows: M4–M64.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 61, f"expected 61 acceptance rows (M4–M64), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.61 ⇒ `0.61.0`, M64, SPEC.md 75)."""
