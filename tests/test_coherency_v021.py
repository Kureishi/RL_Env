...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 67)), sorted(defined)  # A1..A66 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (62 acceptance rows: M4–M65.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 62, f"expected 62 acceptance rows (M4–M65), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.62 ⇒ `0.62.0`, M65, SPEC.md 76)."""
