...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 61)), sorted(defined)  # A1..A60 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (56 acceptance rows: M4–M59.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 56, f"expected 56 acceptance rows (M4–M59), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.56 ⇒ `0.56.0`, M59, SPEC.md 70)."""
