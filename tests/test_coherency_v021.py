...
def test_spec_acceptance_numbers_cited_by_tests():
    ...
    assert defined == set(range(1, 32)), sorted(defined)  # A1..A31 (this round)
    cited: set[int] = set()
...
def test_index_table_rows_resolve():
    ...
    skipped. (27 acceptance rows: M4–M30.)"""
    spec = (REPO / "SPEC.md").read_text(encoding="utf-8")
    ...
    assert rows == 27, f"expected 27 acceptance rows (M4–M30), got {rows}"
...
def test_version_round_v021():
    ...
    `0.21.0`; now v0.27 ⇒ `0.27.0`, M30, SPEC.md 41)."""
